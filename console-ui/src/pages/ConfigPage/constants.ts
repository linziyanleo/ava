export const FIELD_INFO: Record<string, string> = {
  ...
  'agents.defaults.imageGenModel': '图片生成模型，格式 provider/model-name，例如 openai/gpt-image-2',
  ...
  'tools.imageGen.timeout': '图片生成后台任务超时(秒)，默认 300',
  'tools.imageGen.background': '是否把图片生成放入后台任务执行',
  'tools.imageGen.autoContinue': '图片生成完成后是否自动触发 agent continuation，默认关闭',
  'tools.imageGen.autoSend': '图片生成完成后是否自动发送图片到原 channel',
