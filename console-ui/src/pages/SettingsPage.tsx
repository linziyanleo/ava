...
const settingsTree: SettingsNode[] = [
  {
    ...
    icon: Bot,
    children: [
      { to: '/settings/agents-config', label: 'Overview', description: '' },
      {
        ...
        icon: Bot,
        children: [
          { to: '/settings/agents-config/nanobot/config', label: 'Config', description: '' },
          { to: '/settings/agents-config/nanobot/memory', label: 'Memory', description: '', icon: Brain },
          { to: '/settings/agents-config/nanobot/persona', label: 'Persona', description: '', icon: User },
        ],
      },
      {
        ...
        icon: Cpu,
        children: [
          { to: '/settings/agents-config/codex/config', label: 'Config', description: '' },
        ],
      },
      {
        ...
        icon: Monitor,
        children: [
          { to: '/settings/agents-config/claude-code/config', label: 'Config', description: '' },
        ],
      },
      {
        ...
        icon: Image,
        children: [
          { to: '/settings/agents-config/image-gen/config', label: 'Config', description: '' },
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    icon: Shield,
    children: [
      { to: '/settings/system/desktop', label: 'Desktop', description: '', icon: Monitor },
      { to: '/settings/system/lan-access', label: 'LAN Access', description: '', icon: KeyRound },
      { to: '/settings/system/gateway', label: 'Gateway', description: '', icon: Cpu },
      { to: '/settings/system/browser', label: 'Browser', description: '', icon: Monitor },
      { to: '/settings/system/console', label: 'Console', description: '' },
      { to: '/settings/system/version', label: 'Version', description: '', icon: Shield },
    ],
  },
]
...
function SettingsTreeLinks({ nodes }: { nodes: SettingsNode[] }) {
  return (
          ...
          <div key={node.to}>
            ...
            {node.children && (
              <div className="ml-4 mt-1 border-l border-[var(--border)] pl-2">
                <SettingsTreeLinks nodes={node.children} />
              </div>
            )}
          </div>
        )
...
export default function SettingsPage() {
  ...
  const activeRoot = settingsTree.find((item) => isNodeActive(item, location.pathname))
      ...
      <aside className="w-full shrink-0 border-b border-[var(--border)] bg-[var(--bg-secondary)] p-4 md:h-full md:min-h-0 md:w-72 md:overflow-y-auto md:border-b-0 md:border-r">
        ...
        {activeRoot?.children && (
          <div className="mt-5 border-t border-[var(--border)] pt-4">
            ...
            <SettingsTreeLinks nodes={activeRoot.children} />
          </div>
        )}
      </aside>
