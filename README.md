...
它的定位正在从单一 `nanobot` 兼容层扩展为多 Agent 控制平面：`nanobot` 仍是默认 chat backend，但 Console 可以通过显式 slash command 直接启动其他 agent 后台任务，例如 `/codex`、`/claude-code`、`/image-gen`。这条直连路径绕开 nanobot agent loop，任务仍复用 Ava 的 BackgroundTaskStore、状态查询、timeline 和结果回流机制。
...
- `electron/`: P1b macOS `.app` 壳，负责启动 ava-core sidecar 并加载 Console
...
P1b macOS `.app` 壳位于 `electron/`。它会启动本地 `ava-core` sidecar，等待 `/api/gateway/health`，再加载 Console。
...
本阶段生成的 `.app` 仍依赖当前 checkout，移动 `.app` 前需要先完成 P2 打包独立化。
