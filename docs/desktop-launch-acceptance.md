...
- `scripts/verify-desktop-launch.sh` verifies the `CFBundleExecutable` target is executable and reports as a Mach-O binary, then opens the app through LaunchServices and waits for strict Ava startup interfaces: `/api/gateway/health` must report `ready=true`, `shutting_down=false`, and `boot_generation` present, and `/api/auth/me` must return `200` or login-required `401`.
...
Current LaunchServices startup is verified from this Codex session:
...
AVA_DESKTOP_VERIFY_TIMEOUT=120 scripts/verify-desktop-launch.sh
  passed again without clearing the runtime mirror; Console startup interfaces healthy at http://127.0.0.1:6688
```

The packaged app now includes `Contents/Resources/ava-runtime` and mirrors it to `~/Library/Application Support/@ava/electron-shell/runtime-mirror/current` before starting `python -m ava gateway`. This avoids LaunchServices child-process reads from the source checkout during app startup.

Finder/window visual inspection is still session-dependent. Computer Use / Apple Events may be unable to inspect `Ava.app` windows from this environment, so Tray/menu click, cross-app shortcut, Dock badge, update notification click, and Finder reveal visuals still require a normal macOS UI session when closing the full acceptance checklist.
