...
| **P1b** | **Agent Control Plane** — 看得见、管得住         | Agent 注册/状态/配置/单任务派发/线程控制                |
| **P2**  | **Multi-Agent Workflow Runner** — 编得起来    | 线性 pipeline、step 间 artifact 传递、失败重试      |
| **P3**  | **Multi-Agent Coordination Plane** — 自动协同 | 条件分支、循环审批、并行执行、可视化编排                     |


P1 的成功标准：用户能启动/停止/重启 Agent，能观察状态，能派发任务，能取消任务，能查看事件和产物。

## 当前实现 Check List（2026-05-07）
...
- [ ] Agent Detail：为每个 Agent 增加 Config / History / Logs 详情页或抽屉
- [ ] AgentProcessManager：把 start / stop / restart 从 gateway-level restart 推进到 AgentInstance lifecycle
- [ ] Chat Agents Tab：在 Chat 页面实现 `Channel` / `Agents` 顶层视角，支持多 Agent 群聊和 @ 指定 Agent
- [ ] AgentAdapter protocol：把当前 runtime projection 提炼成稳定 `AgentAdapter` 接口，而不是仅在 `AgentRegistryService` 内拼装
- [ ] LAN Access：实现显式开关、PIN/QR 配对、device token、移动端 read_only 默认权限和 audit log
- [ ] Electron shell：验证 Electron main 启动 ava-core sidecar、healthcheck、preload 白名单和打包依赖

### P2 / P3 暂不纳入本轮

- [ ] WorkflowStore / ArtifactStore：workflow run、step、artifact 的结构化存储与 UI
- [ ] 线性 Workflow Runner：step 间 artifact 显式传递、失败重试、coordinator report
- [ ] Coordination Plane：条件分支、循环审批、并行执行、可视化编排
- [ ] HTTPS / mDNS / PWA / Relay：LAN 与远程访问增强
...
- `WorkflowStore`（P2）：Workflow run/step 状态
- `ArtifactStore`（P2）：产物和引用
...
AVA 启动时自动检测本地已安装的 Agent（Claude Code CLI、Codex CLI、Nanobot），已安装的自动带起，未安装的在 Agent Dashboard 上显示下载按钮跳转到对应官网。

检测方式：PATH 查找（`claude` / `codex`）、版本命令验证、Nanobot 使用现有 discovery 机制。检测结果持久化到 `AgentRuntimeStore`，启动时刷新。

### 聊天界面

聊天页面分为两个顶层 Tab：**Channel** 和 **Agents**。

**Channel**：保留当前的 gateway channel 设计（Telegram / Console / CLI / Feishu 等），按消息来源组织 session。这是现有功能的延续。

**Agents**：多 Agent 群聊窗口。

- 显示用户已安装且 AVA 支持的所有 Agent，每个 Agent 一个 Tab 可查看单独对话历史
- 默认只有主 Agent（如 Nanobot）回复每一条消息
- 用户可在单条或选中的多条消息上 @其他 Agent，被 @的 Agent 拿到这些消息作为上下文并回复
- 主 Agent 不回复 @了其他 Agent 的消息，但会拿到上下文（保持全局感知）
- 用户可以定义 Agent 流程（跨能力编排、多 Agent 串行工作流），交给主 Agent 控制执行
- 本质上是一个"多 Agent 群聊"模型，主 Agent 是默认 responder，其他 Agent 按需激活
...
- **artifacts 表**（P2）：产物记录（id / type / uri / metadata / workflow_id / step_id）
...
- 新增页面（Agent Dashboard 等）同步做移动端适配
...
- **P3**：Relay 远程访问、正式 HTTPS、远程 inbox、移动推送
